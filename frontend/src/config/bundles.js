...
export const BUNDLES = [
...

  {
    ...
    type: 'quantity',
    group: 'drinks',
    match: (item) => item.subcategory === 'DRINKS',
    quantity: 4,
    bundlePrice: 20,
  },
  {
  ...
  },
  {
    ...
    type: 'quantity',
    group: 'drinks',
    match: (item) => item.subcategory === 'DRINKS' && !/keef/i.test(item.name),
    quantity: 24,
    bundlePrice: 100,
  },
...

  {
    ...
    type: 'quantity',
    match: (item) => /oranguava|love truffles|ogkb/i.test(item.name) && /3\.5g|1\/8\s*oz/i.test(item.unitWeight),
    quantity: 3,
    bundlePrice: 55,
  },
  {
    ...
    type: 'quantity',
    match: (item) => /high hopes/i.test(item.name) && /3\.5g|1\/8\s*oz/i.test(item.unitWeight),
    schedule: { days: [5] },
    quantity: 2,
    bundlePrice: 28,
  },
...

  {
    ...
    type: 'quantity',
    match: (item) => /hellavated/i.test(item.name) && /1g/i.test(item.unitWeight) && !/cloud bar/i.test(item.name),
    quantity: 2,
    bundlePrice: 45,
  },
  {
